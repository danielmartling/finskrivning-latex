...
\newenvironment{drillsheet}{%
    ...
    \renewcommand{\thelinenumber}{%
        \begin{tikzpicture}[overlay]
                ...
                % uppercase height
                \draw [ultra thin,black!25] (0,\uppercaseheight - 0.1cm) -- ++(\textwidth,0);
                % descender depth
...
\begin{drillsheet}
    ...
    Dolk $\dagger$ $\ddagger$ \\ 
	Oändlighet $\infty$ \\
	Tomma mängden $\emptyset$ $\varnothing$ $\phi$ \\
	Plancks reducerade konstant $\hbar$ \\
	Vinkelrät $\bot$ \\
	Parallell $\|$ \\
	Proportionell $\varpropto$ $\thicksim$ \\
	För varje $\forall$ \\
	Existerar $\exists$ \\
	Alltså $\therefore$ \\
	Ty $\because$ \\
	...
	nabla $\nabla$ \\
	del $\partial$ \\
	Derivata $f'(a) = \left.\frac{\text{d}f}{\text{d}x} \ \right|_{x=a}$ \\
	Integral $\int_a^b f(x) \ \text{d}x = \left[ F(x)\right]_a^b$     
\end{drillsheet}
\end{document}
